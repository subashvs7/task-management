...
import {
  Upload, X, Download, Trash2, Eye, Search, Filter,
  FileText, Image, Film, Music, Archive, File,
  ChevronDown, CheckSquare, MoreVertical, Edit3,
  AlertTriangle, CheckCircle2, Loader2, Grid3X3, List,
  Paperclip, RefreshCw, Info, ExternalLink, Copy,
  FileSpreadsheet, Presentation, Code, ZoomIn,
} from 'lucide-react';
...
interface Props {
  ...
  attachableId: number;
  readOnly?: boolean;
  compact?: boolean;   // minimal mode for sidebars
  showStats?: boolean;
}
...
const CATEGORY_CONFIG: Record<string, {
  ...
  powerpoint:  { label: 'PowerPoint',  icon: Presentation,    color: 'text-orange-600', bg: 'bg-orange-50', border: 'border-orange-200' },
  ...
  code:        { label: 'Code',        icon: Code,            color: 'text-indigo-600', bg: 'bg-indigo-50', border: 'border-indigo-200' },
...
function getFileIcon(category: string, className = 'w-5 h-5') {
  const cfg = CATEGORY_CONFIG[category] ?? CATEGORY_CONFIG.other;
  return <cfg.icon className={`${className} ${cfg.color}`} />;
}

function getFileBg(category: string) {
...
function FilePreviewModal({ attachment, onClose }: {
  ...
  const cfg = getFileBg(attachment.file_category);
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-100">
          <div className="flex items-center gap-3 min-w-0">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${cfg.bg} border ${cfg.border}`}>
              {getFileIcon(attachment.file_category, 'w-5 h-5')}
            </div>
        ...
        <div className="flex-1 overflow-auto bg-gray-50">
          {attachment.mime_type.startsWith('image/') ? (
            ...
            <iframe
              src={attachment.url + '#toolbar=1'}
              className="w-full h-[70vh]"
              title={attachment.original_name}
            />
          ) : attachment.mime_type.startsWith('video/') ? (
            <div className="flex items-center justify-center p-6">
              <video controls className="max-w-full max-h-[70vh] rounded-xl" style={{ maxHeight: '70vh' }}>
            ...
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              <div className={`w-20 h-20 rounded-2xl flex items-center justify-center ${cfg.bg} border-2 ${cfg.border}`}>
                {getFileIcon(attachment.file_category, 'w-10 h-10')}
              </div>
...
function AttachmentCard({ attachment, selected, onSelect, onPreview, onDownload, onEdit, onDelete }: {
  ...
  return (
    <div className={`group bg-white rounded-2xl border shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 overflow-hidden ${
      ...
      {/* Preview area */}
      <div
        className="relative h-36 cursor-pointer overflow-hidden bg-gray-50 flex items-center justify-center"
        onClick={onPreview}
      >
        {/* Selection checkbox */}
        <div className="absolute top-2.5 left-2.5 z-10">
          <input type="checkbox" checked={selected} onChange={onSelect}
            ...
            className="w-4 h-4 rounded accent-blue-600 opacity-0 group-hover:opacity-100 transition-opacity"
        ...
        {attachment.is_image && attachment.url ? (
          ...
          <div className="flex flex-col items-center gap-2">
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center border-2 ${cfg.bg} ${cfg.border}`}>
              {getFileIcon(attachment.file_category, 'w-7 h-7')}
            </div>
    ...
    <tr className={`hover:bg-gray-50 transition-colors group border-b border-gray-50 last:border-0 ${selected ? 'bg-blue-50' : ''}`}>
      ...
      <td className="px-3 py-3 max-w-xs">
        <button onClick={onPreview}
          className="text-sm font-semibold text-gray-900 hover:text-blue-600 transition-colors text-left truncate max-w-full block">
          {attachment.original_name}
        </button>
        {attachment.description && (
          <p className="text-xs text-gray-400 truncate mt-0.5">{attachment.description}</p>
      ...
      <td className="px-3 py-3">
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          {isPreviewable(attachment.mime_type) && (
            <button onClick={onPreview}
              className="p-1.5 rounded-lg hover:bg-blue-50 text-blue-500 transition-colors" title="Preview">
  ...
  return (
    ...
    >
      ...
      <div className={`w-14 h-14 rounded-2xl mx-auto mb-4 flex items-center justify-center transition-all ${
        dragging ? 'bg-blue-500 scale-110' : 'bg-gray-100'
      }`}>
        <Upload className={`w-7 h-7 transition-colors ${dragging ? 'text-white' : 'text-gray-400'}`} />
      </div>

      {dragging ? (
        <p className="text-blue-600 font-bold text-sm">Drop files here!</p>
      ) : (
        <>
          ...
          <p className="text-xs text-gray-400">
            Images, PDFs, Word, Excel, PowerPoint, Videos, Archives and more
          </p>
...
export default function TaskAttachments({
  ...
  }, [attachableId, attachableType, filterCategory, search, paramKey]);
  ...
  if (compact) {
    // Minimal inline version for sidebar / card footer
    return (
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          {!readOnly && (
            <label className="flex items-center gap-1 text-xs font-medium text-blue-600 hover:text-blue-800 cursor-pointer">
        ...
        {loading ? (
          ...
          <div className="space-y-1.5 max-h-48 overflow-y-auto">
            {attachments.map((att) => {
              const cfg = getFileBg(att.file_category);
              return (
                <div key={att.id}
                  className="flex items-center gap-2.5 p-2 bg-gray-50 rounded-xl border border-gray-100 hover:border-blue-200 group transition-colors">
                  <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${cfg.bg}`}>
                    {att.is_image && att.url
                      ? <img src={att.url} alt="" className="w-7 h-7 object-cover rounded-lg" />
                      : getFileIcon(att.file_category, 'w-4 h-4')}
                  </div>
                  <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    {isPreviewable(att.mime_type) && (
                      <button onClick={() => setPreviewItem(att)}
                        className="p-1 rounded-lg hover:bg-blue-50 text-blue-400 transition-colors">
    ...
    <div className="space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          {showUpload && (
            <div className="px-5 pb-5 space-y-4 border-t border-gray-100">
              ...
              <UploadProgress items={uploadItems} onRemove={(id) => setUploadItems((p) => p.filter((i) => i.id !== id))} />
            </div>
          )}
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-50">
          <div className="flex items-center gap-3">
            ...
            {sorted.length > 0 && (
              <button
                onClick={selectedIds.length === sorted.length ? clearSel : selectAll}
          ...
          {selectedIds.length > 0 && (
            <div className="flex items-center gap-2">
              ...
              <button onClick={handleBulkDownload}
                className="flex items-center gap-1.5 text-xs px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg font-medium transition-colors">
                <Download className="w-3.5 h-3.5" /> Download
              </button>
              {!readOnly && (
                <button onClick={() => setConfirmBulkDelete(true)}
                  className="flex items-center gap-1.5 text-xs px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-700 rounded-lg font-medium transition-colors">
                  <Trash2 className="w-3.5 h-3.5" /> Delete
                </button>
              )}
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-gray-100">
          ...
          <p className="text-gray-500 font-semibold text-lg">No attachments</p>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="px-3 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide w-12">Preview</th>
                ...
                <th className="px-3 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">Size</th>
                <th className="px-3 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">Uploaded By</th>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 space-y-4">
            <div className="flex items-start gap-3 p-4 bg-red-50 rounded-xl border border-red-100">
              ...
              <div>
                <p className="text-sm font-semibold text-red-700">Delete attachment?</p>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 space-y-4">
            <div className="flex items-start gap-3 p-4 bg-red-50 rounded-xl border border-red-100">
              <AlertTriangle className="w-5 h-5 text-red-500 flex-shrink-0 mt-0.5" />
              <div>
                <p className="text-sm font-semibold text-red-700">Delete {selectedIds.length} files?</p>
                <p className="text-xs text-red-500 mt-1">This cannot be undone.</p>
              </div>
            </div>
            <div className="flex gap-3">
